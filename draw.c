...
//判別式Dを求める
double get_sp_test_condition(t_vec vec, int x, int y, double r){
	double A;
	double B2;
	double C;
	double D;
	...
	d_vec.y = y + 0.5 - vec.y;
	d_vec.z = 0 - r;
	A = InnerProduct(d_vec, d_vec);
	B2 = InnerProduct(vec, d_vec);
...
double sp_intersection(int x, int y, t_minirt *global_info, t_objects *tmp_o_list){
	...
	t = get_sp_test_condition(pvc, x, y,sp_obj->diameter);
	return (t);
}
