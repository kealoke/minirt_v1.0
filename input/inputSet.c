...
void setAmbient(char **token, t_minirt *scene)
{
  ...
  flag = setRGBcolor(token[2], &(scene->amb->color));
  if (flag == false)
...
void setCamera(char **token, t_minirt *scene)
{
  ...
  if (checkVecRange(scene->cam->ori_vec) == false)
    printErrAndExit(CAM_VEC_ERR);
  scene->cam->fov = ft_atoi(token[3]);
  if (scene->cam->fov < 0 || 180 < scene->cam->fov)
...
void setLight(char **token, t_minirt *scene)
{
  ...
  if (scene->light->brightness < 0.0 || 1.0 < scene->light->brightness)
    printErrAndExit(LIGHT_BRIGHT_ERR);
  flag = setRGBcolor(token[3], &(scene->light->color));
  if (flag == false)
